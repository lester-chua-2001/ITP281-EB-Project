...
public class OrderDriver
{
    ...
    public OrderDriver(string order_id, string pickupdvr,
    ...
    public List<OrderDriver> getOrderAll()
    {
        List<OrderDriver> orderList = new List<OrderDriver>();

        string order_id, pickupdvr, deliverydvr, order_status, payment_info, cust_id, total_price, todate, totime, frodate, frotime, pickupaddrid, deliveryaddrid, email;
        //     
        string queryStr = "select A.cust_id, A.order_id, A.total_price, A.pickupdvr, A.deliverydvr, A.todate, A.totime, A.frodate, A.frotime, A.order_status, A.payment_info, A.pickupaddrid, A.deliveryaddrid, (select email from cust_info D where D.cust_id = A.cust_id) as email from laundry_order A";
        SqlConnection conn = new SqlConnection(_connstr);
        SqlCommand cmd = new SqlCommand(queryStr, conn);
        conn.Open();
        SqlDataReader dr = cmd.ExecuteReader();
